...
import static kr.ac.uos.uos_easy_life.core.model.Department.*;
...
public class CoursePlanner {
  ...
  private List<Course> getRecommendedMajorEssentialCourses(int grade, int semester, int majorCompletedCredit) {
    ...
    if (grade == 1 && semester == 1) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("창의공학기초설계", ComputerScience));
      recommendedCourses.add(courseRepository.findByNameAndDepartment("C언어및실습", ComputerScience));
    } else if (grade == 1 && semester == 2) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("논리회로및실습", ComputerScience));
      recommendedCourses.add(courseRepository.findByNameAndDepartment("이산수학", ComputerScience));
    } else if (grade == 2 && semester == 1) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("자료구조", ComputerScience));
    } else if (grade == 2 && semester == 2) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("운영체제", ComputerScience));
    } else if (grade == 3 && semester == 1) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("소프트웨어공학", ComputerScience));
    } else if (grade == 3 && semester == 2) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("컴퓨터과학종합설계", ComputerScience));
    } else if (grade == 4 && semester == 1 && majorCompletedCredit < 24) {
      recommendedCourses.add(courseRepository.findByNameAndDepartment("컴퓨터과학종합설계", ComputerScience));
    }
  ...
  private List<Course> getRecommendedLiberalEssential(int grade, int semester) {
    if (grade == 1 && semester == 1) {
      ...
      courses.add(courseRepository.findByNameAndDepartment("물리학및실험2", GeneralPhysics));
      courses.add(courseRepository.findByNameAndDepartment("화학및실험2", GeneralChemistry));
      courses.add(courseRepository.findByNameAndDepartment("생물학및실험2", GeneralBiology));
      return courses;
    }
  ...
  public List<Course> planCourses(User user, UserAcademicStatus userAcademicStatus) {
    ...
    // 1학년 1학기의 경우 hard-coded 시간표 반환
    if (user.getCurrentGrade() == 1 && user.getCurrentSemester() == 1) {
      List<Course> courses = new ArrayList<>();
      courses.add(courseRepository.findByNameAndDepartment("과학기술글쓰기", CommunicationClass));
      courses.add(courseRepository.findByNameAndDepartment("대학영어(S)", GeneralEnglish));
      courses.add(courseRepository.findByNameAndDepartment("물리학및실험2", GeneralPhysics));
      courses.add(courseRepository.findByNameAndDepartment("화학및실험2", GeneralChemistry));
      courses.add(courseRepository.findByNameAndDepartment("생물학및실험2", GeneralBiology));
      courses.add(courseRepository.findByNameAndDepartment("수학 II", GeneralMath));
      courses.add(courseRepository.findByNameAndDepartment("창의공학기초설계", ComputerScience));
      courses.add(courseRepository.findByNameAndDepartment("C언어및실습", ComputerScience));
      courses.add(courseRepository.findByNameAndDepartment("공학도의창업과경영", GeneralEducation));
    ...
    // 아몰랑 컴퓨터과학부 수업 전부 추가~~
    courses.addAll(courseRepository.findByDepartment(ComputerScience));
